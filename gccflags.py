...
PARAMS_DEF_PATH = '~/gcc-4.8.0/gcc/params.def'
PARAMS_WORKING_CACHE_FILE = 'cc_params.json'

#run on Compile-Machine
#libc build directory
LIBC_Build = '/home/lvf/wps/libc/build/'
LIBC_Source = '/home/lvf/wps/libc/eglibc-2.19/malloc/malloc.c'
Src_state1 = 'wps-test1'
Src_state2 = 'wps-test2'
WPS_LIB = " nfs@192.168.7.151:/opt/kingsoft/wps-office/office6/ "
mlphost="192.168.7.144"
mlpport=1234
...
argparser.add_argument('--compile-template',
                       ...
                       help='command to compile {source} into {output} with'
                            ' {flags}')
argparser.add_argument('--compile-limit', type=float, default=120,
...
argparser.add_argument('--cc', default='/home/lvf/bin/bin/gcc', help='g++ or gcc')
...
argparser.add_argument('--memory-limit', default=1024 ** 300, type=int,
...
class GccFlagsTuner(opentuner.measurement.MeasurementInterface):
  ...
  def run_baselines(self):
    log.info("baseline perfs -O3=%.4f", *[self.run_with_flags(['-O3','-mssse3','-msse4.2'], None).time])
  ...
  def check_if_flag_works(self, flag, try_inverted=False):
  ...
  def manipulator(self):
    ...
    #m.add_parameter(manipulator.IntegerParameter('-O', 3))
  ...
  def compile_with_flags(self, flags, result_id):
    ...
    cmd = args.compile_template.format(source=args.source, output=output_dir,
                                       ...
                                       cc=args.cc)
    #print ' '.join(flags)
    #lvf, save flags to LIBC_Build/config.status CFLAGS 
    #default: S["CFLAGS"]="-g -O2"
    os.system("sed -i '/^S\[\"CFLAGS\"\]=/cS\[\"CFLAGS\"\]=\""+" -O3 -mssse3 -msse4.2 "+' '.join(flags)+"\"' "+LIBC_Build+"/config.status")
    #modify malloc.c
    os.system("(grep " + Src_state1 + " " + LIBC_Source + " -q )  && sed -i 's/"+Src_state1+"/"+Src_state2+"/' " + LIBC_Source + " || sed -i 's/"+Src_state2+"/" + Src_state1 + "/' "+LIBC_Source)

    #lvf, cd LIBC_Build and make
    os.system("date")
    #p = subprocess.Popen("cd "+LIBC_Build+" && make ", shell=True, stdout=subprocess.PIPE,stderr=subprocess.STDOUT)
    cmd = "cd "+LIBC_Build+" && make "
    t0 = time.time()
