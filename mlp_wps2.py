...
with tf.Session() as sess:
    ...
    if checkpoint and checkpoint.model_checkpoint_path:
        os.system("mv result result.bak")
        ...
        compile_server=socket.socket(socket.AF_INET,socket.SOCK_STREAM,0)
        compile_server.bind((host,compileport)) 
        compile_server.listen(3)
        ...
        while (True):
            ...
            #waiting for COMPILE Machine signal 'hasscp'
            print ("waiting for compile machine signal 'hasscp'...")
            client,ipaddr=compile_server.accept()
            ...
            while (True):
                ...
                if(pred_res[0] == 0):
                    ...
                    print ("2: wps opened.")
                elif pred_res[0] == 3 and stage == 2:
                    t_end = time.time()
                    stage = 3
                    wps_client.send(str("state3").encode())
                    print ("3: wps shows end page of doc.")
                    #get the elapsed time and send it back to COMPILE Machine's opentuner
                    elapse = t_end - t_begin
                    client.send(('%s' % (elapse)).encode())
                    client.close()
